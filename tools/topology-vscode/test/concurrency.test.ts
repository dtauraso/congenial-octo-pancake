...
//   - Input-sourced edges are NOT auto-concurrent — they're driven by
//     the seed sequence (and pendingSeeds for atTick>0); self-pacing
//     would re-fire the last init value forever, conflicting with the
//     deterministic [v0, v1, ...] sequence
...
describe("classifyConcurrentEdges", () => {
  ...
  it("treats gate-type sources and downstream nodes as gated", () => {
    ...
    const c = classifyConcurrentEdges(spec);
    // src→ag: source `src` is not a gate and not downstream of one →
  ...
  it("excludes Input-sourced edges from auto-concurrent classification", () => {
    const spec: Spec = {
      nodes: [
