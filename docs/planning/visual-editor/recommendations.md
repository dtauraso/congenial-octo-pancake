...
| 3 | **High** | Land handoff issue #0 (**run-start signal**). Retires the mount-time `dest.fill` hack in `Wire.tsx` and InputBody's self-started RAF. | M-L | Removes two mount-time hacks; makes tick-0 structural, not lucky. |
