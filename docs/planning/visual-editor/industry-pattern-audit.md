...
- **R4 (M).** Three panels built with `document.createElement` + manual listeners + `style.display` toggling — [src/webview/views.ts:16-64](../../../tools/topology-vscode/src/webview/views.ts), [src/webview/timeline.ts:48-100](../../../tools/topology-vscode/src/webview/timeline.ts), [src/webview/run.ts:12-47](../../../tools/topology-vscode/src/webview/run.ts) → migrate into React components inside the webview tree. Eliminates ~200 LOC of imperative glue and the bridge in R5.
- **R5 (S).** [src/webview/rf/bridge.ts](../../../tools/topology-vscode/src/webview/rf/bridge.ts) shim that registers handlers so the imperative panels can call into React Flow → delete once R4 lands; consume via Context/zustand instead.
- **R6 (M).** Contenteditable inline editors duplicated across [src/webview/rename.ts:19-65](../../../tools/topology-vscode/src/webview/rename.ts) and [src/webview/sublabel.ts:16-61](../../../tools/topology-vscode/src/webview/sublabel.ts) (find label, make editable, select range, keydown commit/cancel) → one React component using a headless inline-edit hook (e.g. `react-contenteditable`) or floating-ui popover. Centralizes Range/Selection boilerplate (R7).
...
- **R17 (S).** Pre-sampled WAAPI keyframes in [src/webview/rf/riding-keyframes.ts](../../../tools/topology-vscode/src/webview/rf/riding-keyframes.ts) → animate a single `offsetDistance` value with WAAPI; query position on-frame via `path.getPointAtLength`. Drops the keyframe queue entirely.
