...
| C6 | **OBSOLETE** | (Subsystem deleted in sim-substrate rebuild; do not extend.) Every `notify({type:"emit"})` registers exactly one Pulse lifetime: `noteEdgePulseStarted(edgeId)` fires synchronously and `noteEdgePulseEnded(edgeId)` fires after the duration governed by the **emitter node type's** animation rule (`ruleForNodeType(srcNode.type).durationMs`), regardless of whether any renderer (AnimatedEdge / fold-halo / headless / future view modes) is subscribed. The simulator's `deferSlotFreeToView` slot release must observe balanced pairs even when no React component mounts for the edge. | `../../../tools/topology-vscode/test/contracts/pulse-lifetime-view-agnostic.test.ts` + `../../../tools/topology-vscode/test/contracts/pulse-duration-per-node-type.test.ts` (drives `notify` with fake timers, asserts `state.activeAnimations` increments synchronously and decrements at +rule.durationMs for ChainInhibitor / ReadGate / unknown-type fallback) | task/pulse-animation-abstraction (e8f6f49); duration-per-type added on task/pulse-rules-per-node; origin: stacking + frame-stall observed when one global duration was used regardless of arc length |
| C7 | **OBSOLETE** | (Subsystem deleted in sim-substrate rebuild; do not extend.) Pulse lifecycle ends on the **first** of two signals: (a) the renderer calls `signalRendererComplete(pulseId)` upon arc completion, or (b) `rule.durationMs` elapses. The loser is a no-op so `noteEdgePulseEnded(edgeId)` fires exactly once per started pulse. Folded / headless edges fall back to the timer; mounted edges complete in real arc time. | `../../../tools/topology-vscode/test/contracts/pulse-renderer-or-timer.test.ts` (renderer-signal-before-timer ends early; timer-alone ends at duration; late renderer signal is no-op) | task/pulse-rules-per-node |
| C8 | **OBSOLETE** | (Subsystem deleted in sim-substrate rebuild; do not extend.) The visual concurrency cap (`rule.maxConcurrentPerEdge`, enforced in `pulse-concurrency.ts` / `tryClaimVisualSlot`) does NOT desynchronise the simulator-side ledger. N emits produce N balanced started/ended pairs in `state.activeAnimations` / `state.activeAnimationsByEdge` even when the renderer chose to draw < N pulses. Slot-release accounting is unaffected by visual coalesce. | `../../../tools/topology-vscode/test/contracts/pulse-coalesce-balanced.test.ts` (4 emits → 4 balanced lifecycles even with cap=1; visual-slot ledger independent of simulator ledger) | task/pulse-rules-per-node |
