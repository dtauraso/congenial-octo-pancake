...
description: Concurrent timeline system with lateral inhibition for competitive partition matching
type: project
---

The system is a dataflow architecture where Go channels and goroutines replace conventional control flow.
...
- **Lateral inhibition** ensures only the strongest matching timeline claims the input — neighbors get suppressed so addition works cleanly
...
**Why:** The topology IS the logic — behavior emerges from wiring, not procedural code.
