---
name: specify-substrate-layer-first
description: State the substrate-layer answer before/alongside the visible-layer spec. Implicit substrate slots get filled with coordinator-shaped defaults because this project's decentralized model is rare in training data.
metadata:
  type: feedback
---

For any new behavior in this repo, state the **substrate-layer**
answer before (or alongside) the visible-layer spec. "There's a
pause button" is a visible-layer spec. "What does a wire do when
paused?" is the substrate-layer spec. The visible spec alone is not
enough.

**Why:** This project's substrate model — decentralized primitives,
...
(cursor, driver, central state). When the substrate-layer question is
...
substrate underneath, so the regression isn't visible until a
...
local substrate primitive (wire / node / slot) do under this
...
[[substrate-vs-coordinator-bias]]: that memory says coordinator-shaped
...
existing global object (gate, driver, cursor), the substrate-layer
