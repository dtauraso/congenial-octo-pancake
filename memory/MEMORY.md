...
- [feedback_run_is_input_only.md](feedback_run_is_input_only.md) — Node bodies use RAF polling (self-woken), not event-driven wakeup. Bodies call `wire.load(value)` (silent no-op when in-flight); do NOT subscribe to wire phase changes.
...
- [feedback_edge_seed_required_for_rings.md](feedback_edge_seed_required_for_rings.md) — Feedback edges in rings need wire.data.seed to break startup deadlock; substrate-r reads it and fills the dest slot once on mount.
