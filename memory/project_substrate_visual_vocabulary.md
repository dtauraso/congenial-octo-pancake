---
name: Substrate visual vocabulary
description: The sim-substrate rebuild only needs two visual primitives — chan→wire and a per-node running indicator (with reloop). Goroutine and select are not separate visual concepts.
type: project
---

The sim-substrate rebuild's visible-state vocabulary collapses to two primitives:
