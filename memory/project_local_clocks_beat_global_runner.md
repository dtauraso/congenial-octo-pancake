...
- **Problem shape.** Pause-as-freeze (capture `t`, rebase on resume) is simple in any substrate. A harder fix wouldn't have benchmarked locality as cleanly.
...
- When the user reports ease/pain on a transport fix, separate the factors before drawing a substrate conclusion. Ask which one the user means.
