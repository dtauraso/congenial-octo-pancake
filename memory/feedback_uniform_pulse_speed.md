...
description: "Pulse speed is a global substrate constant, not a per-wire property — reject per-wire `speed` props even when an analysis doc argues for them."
metadata: 
  node_type: memory
  type: feedback
...
**Why:** the substrate's correctness rests on uniform pulse speed. `partitioned-launching-fog.md` §6 argued per-wire speed was "the strongest single argument for the control-flow model" because it would let "inhibit travel faster than data." David rejected this when implemented — variable wire speed is not a property the substrate should have. The control-flow model formalization still goes through without it (fan-out convergence + doc reframe stand alone).
