...
var REGISTRY = map[string]NodeBind{
	...
	"ReadGate": {
		...
		Outputs: map[string]PortBind{"gated": {Field: "ToGated"}},
	},
	"ChainInhibitor": {
...
func main() {
	...
	if *check {
		fmt.Printf("spec OK: %d nodes, %d edges, %d bytes generated\n", len(spec.Nodes), len(spec.Edges), len(formatted))
		return
	}
...
func generate(spec Spec) (string, error) {
	...
	for _, e := range spec.Edges {
		ctx := fmt.Sprintf("edge %q (%s.%s -> %s.%s)", e.Label, e.Source, e.SourceHandle, e.Target, e.TargetHandle)
	...
	imports["S"] = "github.com/dtauraso/wirefold/nodes/SafeWorker"
...
// writeSpecSummary emits a comment block at the top of the generated file
// listing every node (id, type) and every edge (label, source→target). Lets a
// reader confirm the spec round-tripped correctly without reading the body.
func writeSpecSummary(b *strings.Builder, spec Spec) {
	fmt.Fprintf(b, "//\n// Spec summary: %d nodes, %d edges.\n", len(spec.Nodes), len(spec.Edges))
	idWidth := 0
	for _, n := range spec.Nodes {
		if len(n.ID) > idWidth {
			idWidth = len(n.ID)
