...
class Level():
    ...
    def visit(self, levels, current_level, current_parent_line_id, current_children):
        ...
        for point in parent_point.children:
            ...
            if prev_group_id != -1:
                if prev_group_id != levels[current_level-1].lines[point.line_ref.id].current_group:
...
class F():
    ...
    def makeLevels(self):
        ...
        count = 0
        while True:
            if count >= len(self.levels[0].read_head.sequence):
                break
            count += 1
            i = 0
            level_count = 0
            ...
            while True:
                ...
                print(f"read head level {i}: {[{'parent_line_id': item['parent_line_id'], 'children': [id(i) for i in item['children']]} for item in self.levels[i].read_head.sequence]}")
                ...
                if new_point is None:
                    ...
                    break
                # print(f"len(self.levels): {len(self.levels)} i: {i}")
                # print(f"read head i {i}: {self.levels[i].read_head.i}")
                # print(f"after")
...
def x25():
    ...
    f = F(ReadHead([{"parent_line_id":i, "children": []} for i in [1, 2, 3, 4, 1, 3, 2, 4, 4, 2, 1, 3, 3, 1]]))
