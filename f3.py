class Point():
    ...
    def __init__(self, top=None, bottom=None, prev=None, next=None, parent=None, children=[], line_ref=None, order_id=0, is_expected=False, expected_sequence_length=0, visit_count=1, streak_count=0, point_position=0):
        self.top = top
        self.bottom = bottom
        self.prev = prev
        self.next = next
    ...
    def __str__(self):
        ...
        return f"(point id: {id(self)}, next: {next}, prev: {prev}, is_expected: {self.is_expected}, order_id: {self.order_id}, line id: {self.line_ref.id})"
...
class Line2():
    ...
    def addPoint2(self, point):
        ...
        self.current_point = point
        self.level.visited_points.append(point)
    ...
    def printLine(self):
        ...
        for i, point in enumerate(self.points):
            ...
            print(f"    {point.point_position}: {id(point)}: streak_count: {point.streak_count}, visit_count: {point.visit_count}, next: {next}, prev: {prev}, parent: {parent}")
...
class ReadHead():
    ...
    def setCurrentNumber(self):
        if 0 > self.i or self.i >= len(self.run_length_encoding):
            return
        self.current_number = self.run_length_encoding[self.i]["item"]["parent_line_id"]
        self.current_children = self.run_length_encoding[self.i]["item"]["children"]
        self.current_streak_count = self.run_length_encoding[self.i]["streak_count"]
    ...
    def doneReading(self):
        return self.i >= len(self.run_length_encoding)
    def isLastNumberRead(self):
        return self.i == len(self.sequence) - 1

    def makeCompressedSequence(self):

        self.run_length_encoding.append({"item": self.sequence[0], "streak_count": 0})
        for item in self.sequence:
            if item["parent_line_id"] == self.run_length_encoding[-1]["item"]["parent_line_id"]:
                self.run_length_encoding[-1]["streak_count"] += 1
            else:
                self.run_length_encoding.append({"item": item, "streak_count": 1})
...
class Level():
    ...
    def connectPoints(self):
        ...
        current_item = self.read_head.current_number
        current_children = self.read_head.current_children
        if current_item not in self.lines:                
            self.addLine2(current_item)
        current_point = Point(line_ref=self.lines[current_item], streak_count=self.read_head.current_streak_count)
        if self.start_point is None:
            self.start_point = current_point
        self.lines[current_item].addPoint2(current_point)
        current_point.children = list(current_children)
        # print(f"current_point.children: {current_point.children}")
        # print(f"lines: {self.lines}")
    
        
        for point in current_point.children:
            # print(f"point: {point.line_ref.id}")
            point.parent = current_point
            self.levels[self.id-1].lines[point.line_ref.id].current_point = None
        if self.prev_point is None:
            self.prev_point = self.lines[current_item].current_point
        else:
            self.prev_point.next = self.lines[current_item].current_point
            self.lines[current_item].current_point.prev = self.prev_point
            self.prev_point = self.lines[current_item].current_point

        # self.printLines()
        # print()

    def cutConnectionsByStreakCount(self):
        tracker = self.start_point
        streak_count = tracker.streak_count
        streak_counts_match = True
        while tracker is not None:
            tracker.visit_count += 1
            if tracker.streak_count != streak_count:
                streak_counts_match = False
                streak_count = tracker.streak_count
            tracker = tracker.next
        if not streak_counts_match:
            tracker = self.start_point
            while tracker is not None:
                if tracker.prev is not None:
                    tracker.prev.next = None
                    tracker.prev = None
                tracker = tracker.next

    def cutConnectionsByRepeatSequence(self):
        self.current_number = self.read_head.current_number
        line = self.lines[self.current_number]
        tracker = None
        for point in line.points:
            if point.visit_count == 2:
                tracker = point
                break
        if self.current_number not in self.alphabet:
            self.alphabet[self.current_number] = True
        else:
            self.alphabet = {self.current_number: True}
    ...
    def cutConnectionsVerticallyWithSequenceStart(self):
        self.current_item = self.read_head.current_number
        line = self.lines[self.current_item]
        has_sequence_start = False
        tracker = None
        for point in line.points:
            if point.visit_count == 3:
                if point.prev is None and point.next is not None:
                    has_sequence_start = True
                    break
            ...
            for point in line.points:
                if point.visit_count == 3:
                    if point.prev is not None:
                        # print(f"cut")
                        point.prev.next = None
                        point.prev = None
                        # point.visit_count += 1

    def cutConnectionsVerticallyWithSequenceEnd(self):
        self.current_item = self.read_head.current_number
        line = self.lines[self.current_item]
        has_sequence_end = False
        tracker = None
        for point in line.points:
            if point.visit_count == 3:
                if point.prev is not None and point.next is None:
                    has_sequence_end = True
                    break
        if has_sequence_end:
            # print(f"line id: {line.id}")
            for point in line.points:
                if point.visit_count == 3:
                    if point.next is not None:
                        # print(f"cut")
                        point.next.prev = None
                        point.next = None
                        # point.visit_count += 1
    
    def cutConnectionsDivergingFromSamePath(self):
        self.current_item = self.read_head.current_number
        line = self.lines[self.current_item]
        trackers = [point for point in self.lines[self.current_item].points
                                            if point.visit_count > 0]
        line_has_added_points = len(trackers) > 1
        if not line_has_added_points:
            return
        # print(f"trackers {trackers}")
        prev_line_id = None
        next_line_id = None
        if trackers[0].prev is not None:
            prev_line_id = trackers[0].prev.line_ref.id
        if trackers[0].next is not None:
            next_line_id = trackers[0].next.line_ref.id
        # print(f"{prev_line_id}, {next_line_id}")
        cut_prev_trackers = False
        cut_next_trackers = False
        for tracker in trackers:
            # print(f"tracker: {tracker}\n")
            if tracker.prev is not None:
                # print(f"prev_line_id: {prev_line_id}, tracker.prev.line_ref.id: {tracker.prev.line_ref.id}\n")
                if prev_line_id != tracker.prev.line_ref.id:
                    cut_prev_trackers = True
            if tracker.next is not None:
                # print(f"next_line_id: {next_line_id}, tracker.next.line_ref.id: {tracker.next.line_ref.id}\n")
                if next_line_id != tracker.next.line_ref.id:
                    cut_next_trackers = True
            # print(f"cut_prev_trackers: {cut_prev_trackers}, cut_next_trackers: {cut_next_trackers}\n")
        if cut_prev_trackers or cut_next_trackers:
            for tracker in trackers:
                if cut_prev_trackers:
                    if tracker.prev is not None:
                        tracker.prev.next = None
                        tracker.prev = None
                if cut_next_trackers:
                    if tracker.next is not None:
    ...
    def findSequence(self):
        ...
        if len(self.sequence) == 0:
            ...
            if self.sequence[-1] == tracker.prev:
                self.sequence.append(tracker)
                tracker.visit_count += 1
                if tracker.next is None:
                    if len(self.sequence) > 1:
                        self.sequences.append(self.sequence)
                        self.sequence = []
            elif self.sequence[-1] != tracker.prev:
                if len(self.sequence) == 1:
                    self.sequences.append(self.sequence)
                    self.sequence = [tracker]
                    tracker.visit_count += 1
                if len(self.sequence) > 1:
                    self.sequences.append(self.sequence)
                    self.sequence = []
        if self.read_head.i == len(self.read_head.run_length_encoding) - 1:
            self.sequences.append(self.sequence)
            self.sequence = []

    def checkForOriginalSequence(self, sequence):
        copy_point = sequence[0]
        original_sequence = []
        for i in range(copy_point.point_position):
            trial_point = self.lines[copy_point.line_ref.id].points[i]
            original_sequence = []
            for i, item in enumerate(sequence):
                if (trial_point.next is None or trial_point.prev is None) and (item.next is None and item.prev is None):
                    break
                if trial_point.line_ref.id != item.line_ref.id:
                    break
                elif trial_point.line_ref.id == item.line_ref.id:
                    original_sequence.append(trial_point)
                if trial_point is not None:
                    trial_point = trial_point.next
                else:
                    break
            if len(original_sequence) == sequence:
                break
        return original_sequence
    
    def appendToNextLevelReadHead(self):
        for sequence in self.sequences:
            print(f"sequence {[f'{point.line_ref.id}, {id(point)}' for point in sequence]}")
            original_sequence = self.checkForOriginalSequence(sequence)
            print(f"original sequence {[f'{point.line_ref.id}, {id(point)}' for point in original_sequence]}")
            if len(original_sequence) == 0:
                print("new sequence")
            else:
                print("have original sequence")
            print()
...
class F():
    ...
    def makeLevels2(self):
        
        i = 0 #len(self.levels)
        while i < 1:
            self.levels[i].read_head.makeCompressedSequence()
            [print(f"{point}") for point in self.levels[i].read_head.run_length_encoding]

            while not self.levels[i].read_head.doneReading():
                self.levels[i].read_head.setCurrentNumber()
                self.levels[i].connectPoints()
                self.levels[i].read_head.next2()
            self.levels[i].read_head.restart()
            # for j, level in enumerate(self.levels):
            #     print(f"level {j}\n")
            #     level.printLines()
            #     print("----------")
            
            self.levels[i].cutConnectionsByStreakCount()
            # for j, level in enumerate(self.levels):
            #     print(f"level {j}\n")
            #     level.printLines()
            #     print("----------")
            ...
            while not self.levels[i].read_head.doneReading():
                self.levels[i].read_head.setCurrentNumber()
                self.levels[i].cutConnectionsByRepeatSequence()
                self.levels[i].read_head.next2()
            self.levels[i].read_head.restart()
            # for j, level in enumerate(self.levels):
            #     print(f"level {j}\n")
            ...
            while not self.levels[i].read_head.doneReading():
                self.levels[i].read_head.setCurrentNumber()
                self.levels[i].cutConnectionsVerticallyWithSequenceStart()
                self.levels[i].read_head.next2()
            self.levels[i].read_head.restart()
            # for j, level in enumerate(self.levels):
            #     print(f"level {j}\n")
            ...
            while not self.levels[i].read_head.doneReading():
                self.levels[i].read_head.setCurrentNumber()
                self.levels[i].cutConnectionsVerticallyWithSequenceEnd()
                self.levels[i].read_head.next2()
            self.levels[i].read_head.restart()
            # for j, level in enumerate(self.levels):
            #     print(f"level {j}\n")
            ...
            while not self.levels[i].read_head.doneReading():
                self.levels[i].read_head.setCurrentNumber()
                self.levels[i].cutConnectionsDivergingFromSamePath()
                self.levels[i].read_head.next2()
            self.levels[i].read_head.restart()
            # for j, level in enumerate(self.levels):
            #     print(f"level {j}\n")
            ...
            while not self.levels[i].read_head.doneReading():
                self.levels[i].read_head.setCurrentNumber()
                self.levels[i].findSequence()
                self.levels[i].read_head.next2()
            self.levels[i].read_head.restart()
            print(f"{[[f'{point.line_ref.id}, {id(point)}' for point in x] for x in self.levels[i].sequences]}")
            for j, level in enumerate(self.levels):
                print(f"level {j}\n")
                level.printLines()
                print("----------")

            self.levels[i].appendToNextLevelReadHead()
